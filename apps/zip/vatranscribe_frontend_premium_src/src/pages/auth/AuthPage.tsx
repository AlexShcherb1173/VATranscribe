        ...
        <div className="relative flex h-full flex-col justify-between rounded-[2rem] border border-slate-200 bg-white/70 p-10 shadow-2xl shadow-slate-950/10 backdrop-blur dark:border-white/10 dark:bg-white/[0.05]">
          <div>
            ...
            <h1 className="mt-8 max-w-xl text-5xl font-semibold tracking-[-0.06em]">
              One upload. Transcript, subtitles and content pack.
            </h1>
          ...
          <div className="grid gap-3">
            {["Paste URL → transcript", "Upload file → subtitles", "Transcript → summary + posts"].map((item) => (
              <div key={item} className="rounded-2xl border border-slate-200 bg-white/80 px-4 py-3 text-sm font-medium dark:border-white/10 dark:bg-white/[0.05]">
                {item}
              </div>
            ))}
          </div>
      ...
      <section className="flex items-center justify-center p-5">
        <div className="w-full max-w-md rounded-[2rem] border border-slate-200 bg-white p-6 shadow-xl shadow-slate-950/5 dark:border-white/10 dark:bg-white/[0.04]">
          <div className="flex items-center justify-between gap-3">
            <div>
              <div className="text-xs font-semibold uppercase tracking-[0.24em] text-cyan-700 dark:text-cyan-300">VATranscribe</div>
              ...
              {(["en", "ru"] as const).map((item) => (
                <button
                  ...
                  className={[
                    ...
                    language === item ? "bg-slate-950 text-white dark:bg-cyan-300 dark:text-slate-950" : "text-slate-500",
                  ].join(" ")}
                >
                  {item.toUpperCase()}
                </button>
          ...
          <div className="mt-6 grid grid-cols-2 rounded-2xl border border-slate-200 bg-slate-50 p-1 dark:border-white/10 dark:bg-white/5">
            <button type="button" onClick={() => setTab("login")} className={["rounded-xl px-3 py-2.5 text-sm font-semibold transition", tab === "login" ? "bg-slate-950 text-white dark:bg-white dark:text-slate-950" : "text-slate-500 dark:text-slate-400"].join(" ")}>Login</button>
            <button type="button" onClick={() => setTab("register")} className={["rounded-xl px-3 py-2.5 text-sm font-semibold transition", tab === "register" ? "bg-slate-950 text-white dark:bg-white dark:text-slate-950" : "text-slate-500 dark:text-slate-400"].join(" ")}>Register</button>
          </div>
