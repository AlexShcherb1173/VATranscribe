...
import { formatBytes, formatDate, formatHoursFromSeconds, percentage } from "@/shared/lib/format";
...
function UsageBar({ label, used, limit, value }: { label: string; used: number; limit: number; value: string }) {
  ...
  return (
    <div className="rounded-2xl border border-slate-200 bg-white p-4 dark:border-white/10 dark:bg-white/[0.04]">
      <div className="flex items-center justify-between gap-3 text-sm">
        ...
        <span className={pct >= 80 ? "font-semibold text-amber-600" : "text-slate-500 dark:text-slate-400"}>{value}</span>
      </div>
...
export function DashboardPage() {
  ...
  const usageAlert = quota ? percentage(quota.transcription_seconds_used, quota.transcription_seconds_limit) >= 80 : false;
  ...
  async function handlePasteLink(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      setUrl("");
      toastSuccess("Download queued", "The URL is being processed by the worker.");
      navigate("/app/jobs");
    } catch (error: any) {
      toastError("Download failed", error?.response?.data?.detail || "Could not create download job.");
    } finally {
    ...
    <div className="space-y-8">
      <section className="grid gap-6 xl:grid-cols-[1.2fr_0.8fr]">
        <div className="premium-card overflow-hidden p-6 md:p-8">
          ...
          <form onSubmit={handlePasteLink} className="mt-7 flex flex-col gap-3 rounded-[1.5rem] border border-slate-200 bg-slate-50 p-2 dark:border-white/10 dark:bg-slate-950/60 md:flex-row">
            <input
              value={url}
            ...
            />
            <button type="submit" disabled={isCreatingDownload} className="premium-button min-h-14 disabled:opacity-60">
        ...
        <div className="premium-card p-5 md:p-6">
          <div className="mb-4 flex items-center justify-between gap-3">
            <div>
              <div className="text-sm font-semibold text-slate-950 dark:text-white">{t.common.uploadFile}</div>
              <p className="mt-1 text-xs text-slate-500 dark:text-slate-400">MP3, WAV, MP4, MOV, M4A</p>
            </div>
      ...
      {usageAlert ? (
        <div className="flex flex-col gap-3 rounded-[1.5rem] border border-amber-200 bg-amber-50 p-5 text-amber-950 dark:border-amber-300/20 dark:bg-amber-300/10 dark:text-amber-100 md:flex-row md:items-center md:justify-between">
          ...
          <button type="button" onClick={() => setPricingOpen(true)} className="rounded-2xl bg-amber-950 px-5 py-3 text-sm font-semibold text-white dark:bg-amber-200 dark:text-amber-950">
      ...
      <section className="grid gap-4 md:grid-cols-4">
        {[
          ["Media files", mediaFiles.length.toString()],
          ["Jobs", jobs.length.toString()],
          ["Succeeded", successfulJobs.toString()],
          ["Running", runningJobs.toString()],
        ].map(([label, value]) => (
      ...
      {quota ? (
        <section className="grid gap-4 lg:grid-cols-3">
          ...
          <UsageBar
            ...
            value={`${formatHoursFromSeconds(quota.transcription_seconds_used)} / ${formatHoursFromSeconds(quota.transcription_seconds_limit)}`}
          />
          <UsageBar
            label="Jobs"
      ...
      <section className="grid gap-6 xl:grid-cols-[0.9fr_1.1fr]">
        <div className="premium-card p-5 md:p-6">
          <div className="mb-4 flex items-center justify-between">
            <h2 className="text-lg font-semibold text-slate-950 dark:text-white">{t.dashboard.recentResults}</h2>
            <Link to="/app/transcriptions" className="text-sm font-semibold text-cyan-700 dark:text-cyan-300">View all</Link>
          </div>
          <div className="space-y-3">
            {transcripts.slice(0, 3).map((transcript) => (
              <Link key={transcript.id} to={`/app/results/${transcript.id}`} className="block rounded-2xl border border-slate-200 bg-slate-50 p-4 transition hover:bg-white hover:shadow-md dark:border-white/10 dark:bg-white/[0.03] dark:hover:bg-white/[0.06]">
                <div className="flex items-center justify-between gap-3">
                  <div className="font-medium text-slate-950 dark:text-white">Transcript ready</div>
                  <span className="rounded-full bg-emerald-100 px-3 py-1 text-xs font-medium text-emerald-700 dark:bg-emerald-400/10 dark:text-emerald-200">Ready</span>
                </div>
                <p className="mt-2 line-clamp-2 text-sm text-slate-500 dark:text-slate-400">{transcript.full_text || transcript.id}</p>
                <div className="mt-3 text-xs text-slate-400">{formatDate(transcript.created_at)}</div>
              </Link>
            ))}
            {!transcripts.length ? <div className="rounded-2xl border border-dashed border-slate-300 p-6 text-sm text-slate-500 dark:border-white/10 dark:text-slate-400">{t.dashboard.empty}</div> : null}
          </div>
        </div>

        <div className="premium-card p-5 md:p-6">
          <div className="mb-4 flex items-center justify-between">
            <h2 className="text-lg font-semibold text-slate-950 dark:text-white">Live jobs</h2>
            <Link to="/app/jobs" className="text-sm font-semibold text-cyan-700 dark:text-cyan-300">Open queue</Link>
          </div>
          <div className="space-y-3">
            {recentJobs.map((job) => (
              <div key={job.id} className="rounded-2xl border border-slate-200 bg-slate-50 p-4 dark:border-white/10 dark:bg-white/[0.03]">
                <div className="flex items-start justify-between gap-3">
                  ...
                  <span className="rounded-full bg-slate-950 px-3 py-1 text-xs font-medium capitalize text-white dark:bg-white dark:text-slate-950">{job.status}</span>
                </div>
              </div>
            ))}
            {!recentJobs.length ? <div className="text-sm text-slate-500 dark:text-slate-400">No jobs yet.</div> : null}
          </div>
        </div>
      </section>
